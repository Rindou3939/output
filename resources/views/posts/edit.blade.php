<x-app-layout>
    <div class="max-w-3xl mx-auto px-4 py-6">
        ...
        <form action="{{ route('posts.update', $post->id) }}" method="POST" class="space-y-6 bg-yellow-50 p-4 border-2 border-yellow-600 rounded-md shadow">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea id="description" name="post[description]"
                          class="w-full h-24 rounded border-gray-300 focus:border-yellow-500 focus:ring-yellow-500">{{ old('post.description', $post->description) }}</textarea>
            ...
            <div>
                ...
                <select id="monster_id" name="post[monster_id]"
                    ...
                    @foreach ($monsters as $monster)
                        <option value="{{ $monster->id }}" {{ old('post.monster_id', $post->monster_id) == $monster->id ? 'selected' : '' }}>
                            {{ $monster->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="number" id="recruitment_target" name="post[recruitment_target]"
                       value="{{ old('post.recruitment_target', $post->recruitment_target) }}"
                       class="w-full rounded border-gray-300 focus:border-yellow-500 focus:ring-yellow-500">
